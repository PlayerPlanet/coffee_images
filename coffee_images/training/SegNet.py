...
class Train():
    ...
    @staticmethod
    def Train(trainloader, optimizer, epochs, model=None, path=None):
        """
        Train the SegNet model.
        Args:
            trainloader: DataLoader for training data
            optimizer: torch.optim.Optimizer instance (e.g., Adam, SGD)
            epochs: number of epochs to train
            model: SegNet instance (if None, a new one is created)
        ...
        if path is None:
            ...
            print(f"Creating new checkpoint '{path}'")
        else:
        ...
        for i in range(1, epochs + 1):
            ...
            for j, data in enumerate(tqdm(trainloader, desc=f"Epoch {epoch+1}/{epochs}"), 1):
                ...
                num_batches = j
            if num_batches > 0:
                print(f'Average loss @ epoch: {(sum_loss/num_batches)}')

        print("Training complete. Saving checkpoint...")
        Train.save_checkpoint({'epoch': epoch + epochs, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}, path)
